...
**설명:** [ Learn ]     
• Ch3. Put It Together 에서는 리스트에 항목값으로 딕셔너리를 넣는것을 연습하자.    
• 이 장에서는 리스트를 어떻게 만들고, 사용했는지 복습해보자.
{: .notice--info}
...
**설명:** [ Instruction ]          
• 리스트 studetns 를 만들어라.    
• 그 리스트는 딕셔너리 이름인 lloyd, alice, tyler 을 항목으로 가진다.  
{: .notice--info}
...
**설명:** [ Instruction ]          
• 리스트 students 의 각 학생들 name, homework, quizzes, tests의 값을 출력하라.  
{: .notice--info}
...
**설명:** [ Learn ]    
• Ch6. Just Weight and See 에서는 함수에서 함수를 호출하는 방법을 배워보자.    
• 딕셔너리 cost 의 과일별 평균값을 계산한다.    
• 최종 값을 반환하기 전에 각 과일별 가중 평균값을 곱한 후 반환한다.  
{: .notice--info}
...
• 이 함수는 students 를 입력값을 받는다.          
• 이 함수는 입력 받은 학생별 개별 평균값을 반환한다.     
• 변수 homework 에 학생별 key 가 "homework" 인 것들의 평균값을 계산하여 저장한다.    
• 변수 quizzes  에 학생별 key 가 "quizzes" 인 것들의 평균값을 계산하여 저장한다.    
• 변수 tests    에 학생별 key 가 "test" 인 것들의 평균값을 계산하여 저장한다.    
...
• 학생들의 평균값 "homework", "quizzes","tests" 에 가충치를 준다.    
...
**설명:** [ Hint ]          
• for문을 사용하여 각 학생별 평균값을 사용할때, 변수 student_avg 를 사용하라. 
{: .notice--info}
...
**설명:** [ Instruction ]          
• 리스트 students 를 만들고 그 값으로 학생 alice, lloy, tyler를 넣어라.    
...
**설명:** [ Hint ]          
• 평균점수의 등급을 함수 get_letter_grade 의 결과를 출력한다.
{: .notice--info}
